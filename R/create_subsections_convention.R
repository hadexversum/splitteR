create_subsections_with_convention <- function(dat){
  ...
  results_starts <- lapply(starts, function(start){
    ...
    peptides %>%
      filter(Start == start) %>%
      find_subs_start_with_convention(.)
    
  }) %>% bind_rows()
  # starts the same jesli subunit wiekszy niz 1 albo 2 !!
...
find_subs_end_with_convention <- function(tmp_dat){
  
  # tmp_dat <- filter(peptides, End == 33)
  ...
  if(n_pep == 1){
    ...
    for(i in 1:(n_pep-1)){
      ...
      shorter_omit = stringr::str_count(stringr::str_sub(shorter_pep[["Sequence"]], 1, 2), "[a-z]")
      
      # step by step for now
      longer_sequence <- longer_pep[["Sequence"]]
      shorter_sequence <- shorter_pep[["Sequence"]]
      
      sub_sequence_length <- nchar(longer_pep[["Sequence"]])-nchar(shorter_pep[["Sequence"]]) + shorter_omit
...
find_subs_start_with_convention <- function(tmp_dat){
